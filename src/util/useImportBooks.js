...
export default function useImportBooks({bookTitle=null, loadQty=30, isFirstLoad=true}){
  const dispatch = useDispatch();
  const apiKey = import.meta.env.VITE_API_KEY;
  const firstLoadFlag = bookTitle ? false : isFirstLoad;
  ...
  useEffect(()=>{
      if (data && !loading && !error) {
        ...
        dispatch(fetchedBooksActions.loadBooks({books: processedBooks, firstLoadFlag}));
      }
  }, [data, loading, error, firstLoadFlag, dispatch]);
